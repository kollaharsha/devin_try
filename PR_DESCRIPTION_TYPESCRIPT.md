# Add comprehensive TypeScript schema for service configurations with validation and examples

## Overview

This PR adds comprehensive TypeScript schema definitions for service configurations, providing full type safety and excellent developer experience for the AWS CDK ECS Framework.
...
validateServiceConfig(config); // Throws detailed errors if invalid
```
...
## Files Changed
- ✅ `lib/types/service-config.ts` - Complete TypeScript schema with 300+ lines of types and validation
...
- ✅ `lib/stacks/service-stack.ts` - Updated imports and validation
...
- ✅ All YAML configurations validate correctly (`npm run validate`)
- ✅ CDK synthesis works for dev environment (`cdk synth --context environment=dev`)
- ✅ CDK synthesis works for prod environment with multi-region (`cdk synth --context environment=prod`)
- ✅ Multi-region deployment support confirmed (creates 4 stacks for prod: primary and secondary regions)

## Breaking Changes
None. All existing YAML configurations continue to work without modification. The TypeScript types are additive and provide enhanced developer experience without changing the runtime behavior.
